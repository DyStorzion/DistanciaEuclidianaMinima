...
#include "divide_and_conquer_improve.cpp" 
...
int main(int argc, char *argv[])
{
    ...
    for (n = lower; n <= upper; n += step) {
        ...
        for(i = 0; i < n1; i++)
            puntos.push_back({u_distr(rng), u_distr(rng)});
        ...
        for (i = 0; i < runs; i++) {
            ...
            double distancia = minEuclidianDistance(puntos);
        ...
        time_data << n1 << "," << mean_time << "," << time_stdev << ",";
